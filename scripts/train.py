...
if FLAGS.dataset == 'sunrgbd':
    ...
    if FLAGS.ratio == 1:
        ...
        TRAIN_DATASET = ScannetDetectionDataset('fractional_train', num_points=NUM_POINT,
            augment=True,
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        np.random.seed()
        train_one_epoch()
        if EPOCH_CNT == 0 or EPOCH_CNT % 20 == 19: # Eval every 10 epochs
        ...
        if map > best_map:    
            ...
            torch.save(save_dict, os.path.join(LOG_DIR, 'best_checkpoint_{}.tar'.format(NUM_ITERS_TOTAL)))
            log_string("Best changed from {} to {}".format(best_map,map))
            best_map = map
        else:
            log_string("No change, best is {}".format(best_map))            
