...
def train_one_epoch(net,optimizer,criterion,bnm_scheduler,FLAGS):
    ...
    for batch_idx, batch_data_label in enumerate(FLAGS.TRAIN_DATALOADER):
        ...
        if (batch_idx+1) % batch_interval == 0:
            ...
            FLAGS.TRAIN_VISUALIZER.log_scalars({key:stat_dict[key]/batch_interval for key in stat_dict},
                step=(EPOCH_CNT*len(FLAGS.TRAIN_DATALOADER)+batch_idx)*FLAGS.BATCH_SIZE)
            for key in sorted(stat_dict.keys()):
                log_string(FLAGS.LOGGER,'mean %s: %f'%(key, stat_dict[key]/batch_interval))
                stat_dict[key] = 0
...
# Used for AP calculation during evaluation
    ...
    FLAGS.TEST_VISUALIZER.log_scalars({key:stat_dict[key]/float(batch_idx+1) for key in stat_dict},
        (EPOCH_CNT+1)*len(FLAGS.TRAIN_DATALOADER)*FLAGS.BATCH_SIZE)

    for key in sorted(stat_dict.keys()):
        log_string(FLAGS.LOGGER,'eval mean %s: %f'%(key, stat_dict[key]/(float(batch_idx+1))))
...
def train(FLAGS):
    ...
    for epoch in range(FLAGS.START_EPOCH, FLAGS.MAX_EPOCH):
        ...
        if FLAGS.START_ITER !=0:
            log_string(FLAGS.LOGGER,'Current learning rate: %f'%(get_current_lr_by_iters(epoch,FLAGS)))
        else:
            log_string(FLAGS.LOGGER,'Current learning rate: %f'%(get_current_lr(epoch,FLAGS)))
        log_string(FLAGS.LOGGER,'Current BN decay momentum: %f'%(bnm_scheduler.lmbd(bnm_scheduler.last_epoch)))
        log_string(FLAGS.LOGGER,str(datetime.now()))
        # Reset numpy seed.
        # REF: https://github.com/pytorch/pytorch/issues/5059
        np.random.seed()
        train_one_epoch(net,optimizer,criterion,bnm_scheduler,FLAGS)
        if EPOCH_CNT == 0 or EPOCH_CNT % 20 == 1: # Eval every 2 epochs
        ...
        if curr_map > best_map:
            ...
            torch.save(save_dict, os.path.join(FLAGS.LOG_DIR, 'best_checkpoint_{}.tar'.format(NUM_ITERS_TOTAL)))
            log_string(FLAGS.LOGGER,"Best changed from {} to {}".format(best_map,curr_map))
            best_map = curr_map
        else:
