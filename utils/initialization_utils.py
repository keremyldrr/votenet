...
def initialize_dataloader(FLAGS):
    ...
    if FLAGS.DATASET == 'sunrgbd':
        ...
        TRAIN_DATASET = SunrgbdDetectionVotesDataset('train', num_points=FLAGS.NUM_POINTS,
            augment=True,
            use_color=FLAGS.USE_COLOR, use_height=(not FLAGS.NO_HEIGHT),
            use_v1=(not FLAGS.USE_SUNRGBD_V2))
        TEST_DATASET = SunrgbdDetectionVotesDataset('val', num_points=FLAGS.NUM_POINTS,
            augment=False,
            use_color=FLAGS.USE_COLOR, use_height=(not FLAGS.NO_HEIGHT),
            use_v1=(not FLAGS.USE_SUNRGBD_V2))
        ...
        if FLAGS.RATIO == 1:
            TRAIN_DATASET = ScannetDetectionDataset('train', num_points=FLAGS.NUM_POINTS,
                augment=True,
                use_color=FLAGS.USE_COLOR, use_height=(not FLAGS.NO_HEIGHT),custom_path=FLAGS.CUSTOM_PATH)
        else:
            TRAIN_DATASET = ScannetDetectionDataset('fractional_train', num_points=FLAGS.NUM_POINTS,
                augment=True,
                use_color=FLAGS.USE_COLOR, use_height=(not FLAGS.NO_HEIGHT),ratio=FLAGS.RATIO ,custom_path=FLAGS.CUSTOM_PATH)

        TEST_DATASET = ScannetDetectionDataset('val', num_points=FLAGS.NUM_POINTS,
            augment=False,
            use_color=FLAGS.USE_COLOR, use_height=(not FLAGS.NO_HEIGHT))
        ...
        TEST_DATASET = ScannetDetectionFramesDataset(data_setting,split_set="val",num_points=FLAGS.NUM_POINTS,use_color=False,use_height=True,augment=False)
        TRAIN_DATASET = ScannetDetectionFramesDataset(data_setting,split_set="train",num_points=FLAGS.NUM_POINTS,use_color=False,use_height=True,augment=False)
    else:
        print('Unknown dataset %s. Exiting...'%(FLAGS.DATASET))
        exit(-1)
    # log_string(str(len(TRAIN_DATASET)))
    FLAGS.TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=FLAGS.BATCH_SIZE,
        shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn)
    FLAGS.TEST_DATALOADER = DataLoader(TEST_DATASET, batch_size=FLAGS.BATCH_SIZE,
        shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn)
...
def initialize_model(FLAGS):
    ...
    FLAGS.MODEL = MODEL
    net = Detector(num_class=FLAGS.DATASET_CONFIG.num_class,
...
# Load the Adam optimizer
    optimizer = optim.Adam(net.parameters(), lr=FLAGS.LEARNING_RATE, weight_decay=FLAGS.WEIGHT_DECAY)
    if FLAGS.FIXED_LR !=0:
    ...
    if os.path.exists(  FLAGS.LOG_DIR) and FLAGS.OVERWRITE:
        print('Log folder %s already exists. Are you sure to overwrite? (Y/N)'%(FLAGS.LOG_DIR))
        c = input()
        if c == 'n' or c == 'N':
            print('Exiting..')
            exit()
    ...
    if FLAGS.DUMP_RESULTS:
        if not os.path.exists(FLAGS.DUMP_DIR): os.mkdir(FLAGS.DUMP_DIR)
...
# Load checkpoint if there is any
    ...
    if FLAGS.CHECKPOINT_PATH is not None and os.path.isfile(FLAGS.CHECKPOINT_PATH):
        ...
        log_string(FLAGS.LOGGER, "-> loaded checkpoint %s (epoch: %d)"%(FLAGS.CHECKPOINT_PATH, start_epoch))
