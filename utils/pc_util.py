...
def scene_snapshot(point_cloud, scene_bbox, out_filename):
    scene = trimesh.scene.Scene()
    scene.add_geometry(trimesh.points.PointCloud(point_cloud))
    for box in scene_bbox:
        scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box, None))

    with tempfile.NamedTemporaryFile(suffix=".png") as file_obj:
        scene.save_image(visible=False, resolution=(1080, 1080))
        file_obj.seek(0)
...
def write_oriented_bbox_with_color(scene_bbox, out_filename):
    ...
    def convert_oriented_box_to_trimesh_fmt(box):
        ...
        box_trimesh_fmt = trimesh.creation.box(
            lengths, trns, vertex_colors=(box[7], box[8], box[9])
        )
        return box_trimesh_fmt
