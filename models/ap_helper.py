...
def parse_predictions_with_custom_mask(end_points, config_dict):
    ...
    for i in range(bsize):
        batch_pred_map_cls.append([(pred_sem_cls[i,j].item(), pred_corners_3d_upright_camera[i,j], obj_prob[i,j]) \
            for j in range(pred_center.shape[1]) if pred_mask[i,j]==1])
                # if pred_mask[i,j]==1 and obj_prob[i,j]>config_dict['conf_thresh']:
                #     final_mask[i,j] = 1
...
def parse_predictions_ensemble_only_entropy(mc_samples, config_dict,extension = None):
    ...
    for i in range(bsize):
            ...
            if bsize > 1:
                ...
                batch_pred_map_cls.append([((obj_entropy + cls_entropy)[i,j].item(), pred_corners_3d_upright_camera[i,j], obj_prob[i,j]) \
                    for j in range(pred_center.shape[1]) if pred_mask[i,j]==1 and obj_prob[i,j]>config_dict['conf_thresh']])
            else:
                ...
                batch_pred_map_cls.append([((obj_entropy + cls_entropy)[j].item(), pred_corners_3d_upright_camera[i,j], obj_prob[i,j]) \
                    for j in range(pred_center.shape[1]) if pred_mask[i,j]==1 and obj_prob[i,j]>config_dict['conf_thresh']])
    ...
    obj_and_cls_uncertainties = np.array([np.sum([ (a[0]) for a in batch_pred_map_cls[i]]) for i in range(bsize)]) # a list (len: batch_size) of list (len: num of predictions per sample) of tuples of pred_cls, pred_box and conf (0-1)
    # final_mask = np.zeros_like(pred_mask)
...
def make_box_and_unrotate(end_points,config_dict,rot):
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            # rot = np.eye(3)
            pred_center[i,j],_ = pc_util.rotate_point_cloud(pred_center_upright_camera[i,j],rotation_matrix=rot.T) # do rotation to all points here
            pred_center[i,j] = flip_axis_to_camera(pred_center[i,j])
            pred_corners_3d_upright_camera[i, j],_ = pc_util.rotate_point_cloud(pred_corners_3d_upright_camera[i, j],rotation_matrix=rot.T)
            pred_corners_3d_upright_camera[i, j] = flip_axis_to_camera(pred_corners_3d_upright_camera[i, j]) 
    ...
    # return pred_center
def parse_predictions_augmented(mc_samples, config_dict,extension = None,rotations=None):
    """ Parse predictions to OBB parameters and suppress overlapping boxes
    ...
    pred_corners_3d_upright_camera,pred_center,box_sizes = map(list,zip(*[make_box_and_unrotate(mc_samples[idx],config_dict,rot) for idx,rot in enumerate(rotations)]))
    ...
    pred_corners_3d_upright_camera = torch.Tensor(pred_corners_3d_upright_camera).mean(dim=0).cpu().numpy()
    pred_center = torch.Tensor(pred_center).mean(dim=0).cpu().numpy()
    pred_sem_cls = torch.argmax(end_points['sem_cls_scores'],
                                -1).cpu().numpy()  # B,num_proposal
    sem_cls_probs = softmax(end_points['sem_cls_scores'].detach().cpu().numpy(
    ))  # B,num_proposal,10
    pred_sem_cls_prob = np.max(sem_cls_probs, -1)  # B,num_proposal
...
def parse_groundtruths(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            gt_box_sizes[i, j] = np.linalg.norm(
                corners_3d_upright_camera[0] - corners_3d_upright_camera[1]
            ) * np.linalg.norm(corners_3d_upright_camera[2] -
                               corners_3d_upright_camera[1]) * np.linalg.norm(
                                   corners_3d_upright_camera[4] -
...
def find_matching_boxes_center(proposals):
    ...
    final_proposals = []
    for b in range(B):
        centers1 = torch.from_numpy(np.array([pc_util.point_cloud_to_bbox(bx[1])[:3] for bx in proposals[0][b]])).unsqueeze(0)
        centers2 = torch.from_numpy(np.array([pc_util.point_cloud_to_bbox(bx[1])[:3] for bx in proposals[1][b]])).unsqueeze(0)
        centers3 = torch.from_numpy(np.array([pc_util.point_cloud_to_bbox(bx[1])[:3] for bx in proposals[2][b]])).unsqueeze(0)
        centers4 = torch.from_numpy(np.array([pc_util.point_cloud_to_bbox(bx[1])[:3] for bx in proposals[3][b]])).unsqueeze(0)
        ...
        aggregated_mask = (objectness_mask13 + objectness_mask12 + objectness_mask14 + initial_mask) >= ACCEPTANCE_THRESHOLD
...
def find_matching_boxes_iou(proposals):
    ...
    for b in range(B):
        ...
        bool_aggregated_mask = (iou_mask12 + iou_mask13 + iou_mask14 + initial_mask) >= ACCEPTANCE_THRESHOLD
        ...
        filtered_preds = np.array(proposals[0][b],dtype=np.object)[aggregated_mask]
        eliminated_preds = np.array(proposals[0][b],dtype=np.object)[aggregated_mask_inv]
        final_proposals.append(filtered_preds) 
...
def dump_boxes(boxes,prefix=None):
    ...
    for b in range(batch_size):
        ...
        for j in range(num_boxes):
            boxes_to_be_dumped.append(pc_util.point_cloud_to_bbox(flip_axis_to_depth(boxes[b][j][1])))
...
def aggregate_predictions(batch_pred_map_cls_multiple,rotations,dump_dir=None):
    ...
    for b in range(batch_size):
        ...
        for idx,rot in enumerate(rotations): 
            all_boxes.append([])
            pred_boxes_in_scene = [box_tuple[1] for box_tuple in batch_pred_map_cls_multiple[idx][b]]
            stacked_boxes = flip_axis_to_depth(np.vstack(pred_boxes_in_scene))
            # stacked_boxes = np.vstack(pred_boxes_in_scene)
            rotated_boxes,_ = pc_util.rotate_point_cloud(stacked_boxes,rotation_matrix=rot.T) #unrotate stacked_boxes
    ...
    final_proposals,proposals_inv = find_matching_boxes_iou(batch_pred_map_cls_multiple[:])
