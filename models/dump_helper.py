...
def dump_results_for_sanity_check(end_points, dump_dir, config, inference_switch=False):
    ''' Dump results.
    ...
    for i in range(batch_size):
        ...
        if len(obbs)>0:
            ...
            pc_util.write_oriented_bbox(p_obbs, os.path.join(dump_dir, '%06d_gt_bbox_%d.ply'%(idx_beg+i,idx)))
...
def dump_results_with_color(end_points, dump_dir, config, inference_switch=False):
    ...
    cls_entropy_color_map = viridis((cls_entropy - cls_entropy.min())/(cls_entropy.max()-cls_entropy.min()))
    obj_entropy_color_map = viridis((obj_entropy - obj_entropy.min())/(obj_entropy.max()-obj_entropy.min()))
    boxsize_entropy_color_map = viridis((box_size_var - box_size_var.min())/(box_size_var.max()-box_size_var.min()))
    for i in range(batch_size):
        pc = point_clouds[i,:,:]
        objectness_prob = softmax(objectness_scores[i,:,:])[:,1] # (K,)
        ...
        for idx,colormap in enumerate([cls_entropy_color_map,obj_entropy_color_map,boxsize_entropy_color_map,obj_prob_map]):
            obbs = []
            print(names[idx])
            for j in range(num_proposal):
                obb = config.param2colorobb(pred_center[i,j,0:3], pred_heading_class[i,j], pred_heading_residual[i,j],
                                pred_size_class[i,j], pred_size_residual[i,j],colormap[j])
                obbs.append(obb)
            if len(obbs)>0:
                obbs = np.vstack(tuple(obbs)) # (num_proposal, 7)
                pc_util.write_oriented_bbox_with_color(obbs[objectness_prob>DUMP_CONF_THRESH,:], os.path.join(dump_dir, names[idx] + '%06d_pred_confident_bbox.ply'%(idx_beg+i)))
                pc_util.write_oriented_bbox_with_color(obbs[np.logical_and(objectness_prob>DUMP_CONF_THRESH, pred_mask[i,:]==1),:], os.path.join(dump_dir, names[idx] + '%06d_pred_confident_nms_bbox.ply'%(idx_beg+i)))
                pc_util.write_oriented_bbox_with_color(obbs[pred_mask[i,:]==1,:], os.path.join(dump_dir, names[idx] + '%06d_pred_nms_bbox.ply'%(idx_beg+i)))
                pc_util.write_oriented_bbox_with_color(obbs, os.path.join(dump_dir, names[idx] + '%06d_pred_bbox.ply'%(idx_beg+i)))
                pc_util.write_oriented_bbox_with_color(obbs[iou_mask==1,:], os.path.join(dump_dir, names[idx] + '%06d_iou_masked_bbox.ply'%(idx_beg+i)))
