...
from pandas.core.dtypes.cast import _maybe_promote_cached
import torch
...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    if end_points["log_vars"] is None:
        ...
        for batch in range(gt_center.shape[0]):
            ...
            for ii in range(nb_in_frame):
                proposals = log_vars[
                    batch, (ind1[batch] == ii) & (objectness_label[batch] == 1)
                ]
                objectness_scores = objectness_label[batch, (ind1[batch] == ii)]
...
def get_loss(end_points, config):
    ...
    end_points["center_nll"] = center_nll
    box_loss = (
        center_loss
